...
class UiDescriptorGenerator(Generator):
    ...
    def generate(self) -> Dict[str, Any]:
        """
        Build the UI descriptor for exactly one class: self.top_class.

        Dispatched classes (reached via a dispatch slot's dispatches_to map,
        see _get_dispatch_enum) are NOT nested inline here — each one is its own
        separate class, generated by its own separate UiDescriptorGenerator
        instance and written to its own file. See
        generate_all_ui_descriptors() for the orchestration that produces a
        complete, multi-file set (roots + shared category files) in one run.
        """
        self._resolve_top_class()
        return {self.top_class: self._build_descriptor(self.top_class)}

    def serialize(self, output_dir: str = ".", **kwargs) -> None:
        """
        Write three files to output_dir:
          <top_class>.ui.json  — the UI descriptor for this class alone
        ...
        For a complete multi-file set (all roots + every shared category
        file, with dispatches_to filenames resolving correctly across all of
        them), use generate_all_ui_descriptors() instead — this method only
        ever produces output for self.top_class.
        """
        ...
        with open(descriptor_path, "w", encoding="utf-8") as f:
            json.dump(descriptor[self.top_class], f, indent=2, ensure_ascii=False)
    ...
    def _get_dispatch_enum(self, slot: SlotDefinition):
        """
        Return the enum definition backing this slot's dispatch, or None if
        this isn't a dispatch slot.

        A slot is a dispatch slot purely by schema content: its range is an
        enum, and at least one of that enum's permissible values carries a
        dispatch_to annotation (e.g. SortingCategoryEnum, BaseCategoryEnum —
        core.yaml). No naming convention, no designates_type, no root-name
        branching — a fifth root just needs category: range: <SomeEnum> with
        dispatch_to annotations on that enum's values, and this generically
        picks it up.
        """
        ...
        if not slot_range:
            return None
        try:
            enum_def = self.schemaview.get_enum(slot_range)
        except Exception:
            return None
        if enum_def is None or not getattr(enum_def, "permissible_values", None):
            return None
        has_dispatch = any(
            "dispatch_to" in (getattr(pv, "annotations", None) or {})
            for pv in enum_def.permissible_values.values()
        )
        return enum_def if has_dispatch else None

    def _build_dispatch_map(self, dispatch_enum) -> Dict[str, str]:
        """
        Build {category_value: filename} from dispatch_enum's permissible
        values, each carrying a dispatch_to annotation naming the target
        class. category_value keys are the enum's own permissible value
        names; filenames are "<dispatch_to value>.ui.json".
        """
        mapping: Dict[str, str] = {}
        for pv_name, pv in dispatch_enum.permissible_values.items():
            ann = getattr(pv, "annotations", None) or {}
            target = ann.get("dispatch_to")
            if target is None:
                self.logger.warning(
                    f"{dispatch_enum.name} value '{pv_name}' missing "
                    f"'dispatch_to' annotation — skipped in dispatch map"
                )
                continue
            mapping[pv_name] = f"{target.value}.ui.json"
        return mapping
    ...
    LABEL_KEYS = frozenset({"label_en", "label_de"})

    def _get_class_label_annotations(self, class_name: str) -> Dict:
        """
        Return the annotations dict of the first ancestor of class_name
        (walking class_ancestors, same pattern as _get_completeness) that
        declares label_en or label_de.

        This is what lets a single label_en/label_de/aliases_en/aliases_de
        declaration on a Tier 1 category mixin (e.g. FurnitureCategory)
        ...
        for ancestor in self.schemaview.class_ancestors(class_name):
            try:
                ...
                continue
            a = getattr(cls, "annotations", None) or {}
            if any(k in a for k in self.LABEL_KEYS):
                return a
        return {}
    ...
    def _collect_labels(
        ...
        for class_name, class_desc in descriptor.items():
            # Class label — ancestor-walked, see _get_class_label_annotations.
            a = self._get_class_label_annotations(class_name)
    ...
    def _build_descriptor(self, class_name: str) -> Dict[str, Any]:
        ...
        dispatches_to is emitted on any slot whose range is a dispatch enum
        (see _get_dispatch_enum) — an enum with dispatch_to annotations on
        its permissible values — regardless of which class declares it.
        ...
        # Built lazily — only when a dispatch slot is encountered
        # (see _get_dispatch_enum).
        ...
        for slot in slots:
            ...
            dispatch_enum = self._get_dispatch_enum(slot)
            if dispatch_enum is not None:
                if dispatch_map is None:
                    dispatch_map = self._build_dispatch_map(dispatch_enum)
...
ROOTS = ("DonationItem", "StorageCollection", "SortedCollection", "DemandSignal")


def generate_all_ui_descriptors(schema_path: str, output_dir: str, roots=ROOTS) -> None:
    """
    Generate a complete set of UI descriptor files in one run: every root's
    own file, plus every shared category file each root's dispatch slot
    resolves to — auto-discovered by scanning every enum in the schema for
    dispatch_to annotations (e.g. SortingCategoryEnum, BaseCategoryEnum in
    core.yaml), not a hardcoded list and not tied to any specific enum name.

    A category dispatched to by more than one root (every category reached
    via SortingCategoryEnum is reachable from all three physical-item roots)
    is generated once and shared — this is the single-source-of-truth
    sharing the multi-root design exists for, not duplicated per root.

    Also writes labels-en.json / labels-de.json, merged across every
    generated class — safe because _collect_labels derives each label from
    schemaview annotations keyed by name only, never per-root structure.
    """
    os.makedirs(output_dir, exist_ok=True)
    sv = SchemaView(schema_path)

    classes_to_generate = set(roots)
    for enum_def in sv.all_enums().values():
        for pv in (getattr(enum_def, "permissible_values", None) or {}).values():
            ann = getattr(pv, "annotations", None) or {}
            if "dispatch_to" in ann:
                classes_to_generate.add(ann["dispatch_to"].value)

    merged: Dict[str, Any] = {}
    last_gen: Optional[UiDescriptorGenerator] = None
    for class_name in sorted(classes_to_generate):
        gen = UiDescriptorGenerator(schema_path, top_class=class_name)
        descriptor = gen.generate()
        descriptor_path = os.path.join(output_dir, f"{class_name}.ui.json")
        with open(descriptor_path, "w", encoding="utf-8") as f:
            json.dump(descriptor[class_name], f, indent=2, ensure_ascii=False)
        merged.update(descriptor)
        last_gen = gen

    for locale in UiDescriptorGenerator.SUPPORTED_LOCALES:
        labels = last_gen._collect_labels(merged, locale)
        labels_path = os.path.join(output_dir, f"labels-{locale}.json")
        with open(labels_path, "w", encoding="utf-8") as f:
            json.dump(labels, f, indent=2, ensure_ascii=False)
...
def main(argv: Optional[List[str]] = None) -> None:
    """
    CLI entry point (registered as `generate-ui-descriptors` in
    pyproject.toml `[project.scripts]`) — a deliberate, separate action from
    running the test suite. Usage:

        generate-ui-descriptors <schema_path> <output_dir> [--roots ROOT ...]
    """
    parser = argparse.ArgumentParser(
        description="Generate UI descriptor JSON files for all roots and shared category files."
    )
    parser.add_argument("schema_path", help="Path to the root LinkML schema YAML file.")
    parser.add_argument("output_dir", help="Directory to write <ClassName>.ui.json and labels-*.json into.")
    parser.add_argument(
        "--roots",
        nargs="+",
        default=list(ROOTS),
        help=f"Root classes to generate (default: {' '.join(ROOTS)}).",
    )
    args = parser.parse_args(argv)
    generate_all_ui_descriptors(args.schema_path, args.output_dir, roots=tuple(args.roots))
